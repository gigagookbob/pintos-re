...
/* 사용자 가상 페이지(UPAGE)에서 커널 가상 주소 KPAGE로 식별되는
 * 물리 프레임으로 가는 PML4(Page Map Level 4) 매핑을 추가합니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다.
 * KPAGE는 보통 palloc_get_page()로 사용자 풀(user pool)에서 얻은
 * 페이지여야 합니다.
 * WRITABLE이 true이면 새 페이지는 읽기/쓰기 가능(read/write),
 * 그렇지 않으면 읽기 전용(read-only)입니다.
