...
static void populate_pools(struct area *base_mem, struct area *ext_mem)
{
    ...
    uint64_t user_pages =
        total_pages / 2 > user_page_limit ? user_page_limit : total_pages / 2;
    ...
    for (i = 0; i < mb_info->mmap_len / sizeof(struct e820_entry); i++)
    {
        ...
        {
            uint64_t start =
                (uint64_t) ptov(APPEND_HILO(entry->mem_hi, entry->mem_lo));
            uint64_t size = APPEND_HILO(entry->len_hi, entry->len_lo);
            uint64_t end = start + size;
            ...
            start = (uint64_t) pg_round_up(
                start >= usable_bound ? start : usable_bound);
        split:
...
uint64_t palloc_init(void)
{
    ...
    printf("\tbase_mem: 0x%llx ~ 0x%llx (Usable: %'llu kB)\n", base_mem.start,
           base_mem.end, base_mem.size / 1024);
...
/* 하나의 빈 페이지를 확보하여 해당 페이지의 커널 가상 주소를 반환합니다.
   PAL_USER가 설정되어 있으면 사용자 풀(user pool)에서 페이지를 가져오고,
   그렇지 않으면 커널 풀(kernel pool)에서 가져옵니다.
   FLAGS에 PAL_ZERO가 설정되어 있으면, 페이지 내용을 0으로 채웁니다.
   사용 가능한 페이지가 없으면 NULL 포인터를 반환합니다.
   단, FLAGS에 PAL_ASSERT가 설정되어 있으면 NULL을 반환하는 대신
   커널이 패닉(panic) 상태에 빠집니다. */
